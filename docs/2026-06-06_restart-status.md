...
홈에서 실제 사용자가 누르는 흐름으로 예약 결제 화면까지 도달하는 Playwright E2E를 추가했다.
...
- E2E는 테스트 유저/차량을 준비한 뒤 실제 UI에서 `홈 -> 지점 예약하기 -> 작업 선택 -> 시간 선택 -> 안전 동의 -> 결제 화면`까지 클릭한다.
- 안전 동의는 영상 시청 처리, 선택 작업 한정 동의, 체크리스트 4개, 최종 동의를 모두 실제 checkbox/button interaction으로 통과한다.
- 결제 화면 진입까지만 검증하고, 실제 예약 확정 결제 confirm은 기존 checkout/payment E2E가 담당한다.
...
- 이 테스트는 실제 Supabase 테스트 유저와 활성 차량을 준비하지만, 결제 화면 진입까지만 확인하므로 예약 row를 생성하지 않는다.
