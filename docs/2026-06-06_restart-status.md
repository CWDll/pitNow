...
## 48. 2026-06-21 예약-체크인 UI 클릭 E2E 추가

홈에서 실제 사용자가 누르는 흐름으로 예약 결제 화면에 진입하고, FAKE 결제로 예약 완료 후 체크인/이용 중 진입까지 도달하는 Playwright E2E를 추가했다.
...
- E2E는 테스트 유저/차량을 준비한 뒤 실제 UI에서 `홈 -> 지점 예약하기 -> 작업 선택 -> 시간 선택 -> 안전 동의 -> 결제 화면 -> 예약 완료 -> 체크인 -> 이용 중`까지 클릭한다.
...
- Playwright에서 Supabase Storage 업로드 요청만 mock 처리하고, `/api/checkin`과 `/api/reservations/:id/start`는 실제 API/DB로 검증한다.
...
- `reservation_status_logs`에서 `CHECKED_IN`, `IN_USE` 상태 로그 순서를 확인한다.
- 검증 후 테스트 예약은 `CANCELLED`, 테스트 payment는 `REFUNDED`로 cleanup한다.
