...
## 48. 2026-06-21 예약 진입 UI 클릭 E2E 추가

홈에서 실제 사용자가 누르는 흐름으로 예약 결제 화면에 진입하고, FAKE 결제로 예약 완료까지 도달하는 Playwright E2E를 추가했다.
...
- E2E는 테스트 유저/차량을 준비한 뒤 실제 UI에서 `홈 -> 지점 예약하기 -> 작업 선택 -> 시간 선택 -> 안전 동의 -> 결제 화면 -> 예약 완료`까지 클릭한다.
