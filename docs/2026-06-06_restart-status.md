...
홈에서 실제 사용자가 누르는 흐름으로 예약 결제 화면에 진입하고, FAKE 결제로 예약 완료 후 체크인/이용 중/체크아웃/추가정산/이용 완료까지 도달하는 Playwright E2E를 추가했다.
...
- E2E는 테스트 유저/차량을 준비한 뒤 실제 UI에서 `홈 -> 지점 예약하기 -> 작업 선택 -> 시간 선택 -> 안전 동의 -> 결제 화면 -> 예약 완료 -> 체크인 -> 이용 중 -> 체크아웃 -> 추가 정산 -> 이용 완료`까지 클릭한다.
...
- 검증 후 테스트 예약은 `CANCELLED`, 테스트 payments는 `REFUNDED`, checkin row는 cleanup한다.
