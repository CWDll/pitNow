...
• 선택 작업 중 확인 제공 작업이 하나라도 있으면 작업 확인을 신청할 수 있다.
• 기본 5,000원과 확인 제공 작업의 가산 비용만 합산하고, 결제 전에 확인 포함·제외
  작업을 모두 표시한다.
• 예약 작업 스냅샷에는 실제 과금된 작업만 확인 항목과 가산 비용을 저장한다.
...
규칙:
• 확인 항목과 문구는 Admin이 관리하는 전역 카탈로그를 원천으로 사용한다.
• Partner-admin은 작업 단위 제공 여부를 설정하고 해당 작업의 전체 확인 항목을
  조회할 수 있지만 임의 항목을 추가·수정하지 않는다.
