...
| 40 | 정비사 작업 확인 결과 | 작업 확인을 선결제한 완료 예약 상세 | 결과 대기/미진행/1차/재확인 결과와 항목 메모 표시, 예약 완료는 결과에 의해 차단되지 않음 | 자동 검증 성공·배포 재확인 | 예약 스냅샷·PENDING 생성, Partner 결과 등록과 User `RECORDED` 조회 E2E 통과 |
| 41 | 정비사 작업 확인 대상 선택 | 확인 가능한 SELF 작업을 2개 이상 선택 후 작업 확인 활성화·일부 해제·결제 | 활성화 시 전체 기본 선택, 일부 해제 가능, 기본료 1회 + 남은 작업 가산액만 과금, Partner 결과에는 결제한 작업만 표시 | 자동 검증 성공·배포 재확인 필요 | 명시적 `workCheckTaskIds` 서버 검증과 예약 스냅샷 확인 |
