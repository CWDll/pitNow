...
| 11 | 정비소 제공 방식 migration | `20260804_partner_service_modes.sql` 적용 | 대기 | `partners.supports_self_service`, `supports_shop_service` 추가 및 샘플 SELF/SHOP 전용 정비소 설정 |
...
| 17 | Self 체크아웃 | checklist + 2 photos | 작업 확인 재선택 없이 예약 기본요금/작업 확인 비용/추가요금/추가 결제비용 표시 | 성공 | iPhone 체크아웃 확인 |
...
| 37 | SELF 공통 안전교육 | 미완료 계정으로 SELF 안전 동의 진입, `/guide`·`/mypage`에서 독립 진입 | 최초 1회 공통교육 완료 전 결제 진행 불가, 완료 후 원래 화면으로 복귀 | 자동 검증 성공·배포 재확인 | 콘텐츠 버전별 완료 테이블과 예약 생성 서버 검증, 독립 교육 진입점 적용 |
| 38 | DB 기반 SELF 작업 카드 | `/partner/:id/work` SELF 작업 목록 | DB 작업 설명·난이도·확인 항목·작업 확인 비용이 표시 | 자동 검증 성공·배포 재확인 | 430×932 화면에서 5개 DB 작업과 항목 목록 확인, 정적 mock 배열 미사용 |
| 39 | 작업별 안전 동의 | 복수 SELF 작업 선택 후 안전 동의 | 선택한 모든 작업 안전카드와 활성 필수 영상 확인 후 체크박스 동의 | 자동 검증 성공·배포 재확인 | 모바일 예약 E2E 통과, 캔버스 서명 미사용 |
| 40 | 정비사 작업 확인 결과 | 작업 확인을 선결제한 완료 예약 상세 | 결과 대기/미진행/1차/재확인 결과와 항목 메모 표시, 예약 완료는 결과에 의해 차단되지 않음 | 성공 | Partner 1차 결과·메모 저장 후 User 지난 이용 상세에서 동일한 선택 작업 결과가 표시되고 예약 완료를 차단하지 않는 것 확인 |
| 41 | 정비사 작업 확인 대상 선택 | 확인 가능한 SELF 작업을 2개 이상 선택 후 작업 확인 활성화·일부 해제·결제 | 활성화 시 전체 기본 선택, 일부 해제 가능, 기본료 1회 + 남은 작업 가산액만 과금, Partner 결과에는 결제한 작업만 표시 | 성공 | 3개 작업 중 확인 가능한 2개에서 1개만 선택해 기본 5,000원 + 작업 가산 2,000원 결제. Partner 결과 등록과 User 지난 이용 조회에서 선택 작업만 표시되고 제외 작업은 비용·결과에 미포함 확인 |
| 42 | 홈 SELF/SHOP 제공 정비소 필터 | `/`에서 SELF와 SHOP을 번갈아 선택 | 선택한 방식을 실제 제공하는 정비소만 지도·목록·개수에 표시되고 해당 방식 가격으로 정렬 | 자동 검증 성공·migration 적용 후 배포 재확인 | 샘플 서초는 SELF 전용, 송파는 SHOP 전용으로 설정 |
| 43 | 정비소 사진 인라인 넘기기 | `/partner/:id` 사진 영역을 좌우로 스와이프 | 상세보기를 열지 않아도 사진 이동, 현재 순서 표시. 선택 시 기존 확대·좌우 버튼 유지 | 자동 검증 성공·iPhone 재확인 | 모바일 Playwright에서 1/3에서 2/3 전환 확인 |
...
| 21 | 신규 패키지 생성 요청 | 패키지 섹션에서 이름/시간/가격/사유 제출 | 승인 대기 카드 표시 후 `/admin/packages` 신규 생성 요청에 표시 | 자동 검증 성공·배포 재확인 | 입력 라벨, 5분 단위 시간, pending 표시 확인. 처리 결과는 24시간 후 요청 카드에서 사라지고 승인 패키지는 일반 카드로 유지 |
...
| 26 | 종료 예약 노쇼 전환 | 종료 시각이 지난 `CONFIRMED` 예약 조회 또는 노쇼 메모 작성 | User/Partner/Admin 모두 `노쇼` 표시, `CONFIRMED → NO_SHOW` SYSTEM/PARTNER 로그 기록, 자동 환불 없음 | 성공 | migration 적용 및 기존 09:00/12:00 예약 backfill, 배포 URL 표시 확인. Partner API E2E에서 SYSTEM/PARTNER 전환 로그 통과 |
| 27 | 예약 차단 폼 정렬 | 범위·시작·종료·사유·추가 버튼 확인 | 모든 입력 컨트롤이 같은 y축 기준선과 40px 높이로 정렬 | 자동 검증 성공·배포 재확인 | Playwright에서 날짜/시각 포함 7개 컨트롤의 y좌표 차이 1px 이내 확인 |
| 28 | SELF 작업 확인 제공 설정 | 패키지/가격 섹션에서 작업별 제공 상태 변경 | 작업별 전체 확인 항목이 표시되고, 선택 작업 중 제공 가능한 작업만 확인·과금 | 수정 후 재확인 | 제공 작업이 하나 이상이면 옵션 활성화, 포함·제외 작업과 금액 표시. 항목은 Admin 카탈로그 원천 유지 |
| 29 | SELF 작업 확인 결과 등록 | 작업 확인 예약 상세에서 1차/재확인 저장 | 1차 저장 전 재확인 불가, 진행 상태와 전체 미진행 범위가 명확하며 예약 상태는 변경되지 않음 | 수정 후 재확인 | UI와 API 모두 1차 선행 강제, 실제 항목별 1차·재확인 저장과 User 조회 E2E |
| 30 | 기능별 Partner-admin 라우트 | 사이드바의 예약·사진·인증·베이·패키지·차단 메뉴 이동 | `#` 앵커 없이 각 `/partner-admin/...` URL로 이동하고 해당 기능만 표시 | 자동 검증 성공·배포 재확인 | 새로고침·로그인 복귀에도 현재 기능 URL 유지 |
