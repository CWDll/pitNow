...
| 35 | Shop 체크인·작업 상태 | Shop 결제 완료 후 정비소 QR/코드 인증, Partner 작업 시작·완료 | User 인증 시 CHECKED_IN, Partner 시작 시 IN_USE, Partner 완료 시 COMPLETED. User에는 상태 조회만 표시 | 자동 검증 성공·현장 재확인 | QR/코드 인증, 역할별 403, 최종 정산·상태·감사 로그 E2E |
...
| 24 | 현장 체크인 인증정보 | QR 표시, 수동 코드 복사, 재발급 | QR/코드 모두 사용자 인증 성공, 재발급 후 기존 값 거부 | 자동 검증 성공·UI 재확인 | Partner별 고정 인증정보, QR/수동 코드 성공 및 오류 코드 거부 E2E |
