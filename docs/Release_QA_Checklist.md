...
| 14 | Self 체크인 | 예약 시작 15분 전 이후 정비소 QR 또는 수동 코드 + 4 photos | 다른 정비소/오류 코드는 거부, 사진 4장 없으면 진행 불가, 완료 시 CHECKED_IN | 자동 검증 성공·현장 재확인 | Partner 고정 QR·수동 코드 서버 검증 및 Self 전체 UI E2E |
...
| 35 | Shop 체크인·작업 상태 | Shop 결제 완료 후 정비소 QR/코드 인증, Partner 작업 시작·완료 | User 인증 시 CHECKED_IN, Partner 시작 시 IN_USE, Partner 완료 시 COMPLETED. User에는 상태 조회만 표시 | 수정 후 현장 재확인 | QR 일반 카메라는 업체 상세로 이동, 15분 전 CTA/직접 URL 차단, Partner 상세 패키지 표시까지 재검증 |
...
| 24 | 현장 체크인 인증정보 | QR 표시, 수동 코드 복사, 재발급 | QR/코드 모두 사용자 인증 성공, 재발급 후 기존 값 거부 | 수정 후 현장 재확인 | 새 QR은 일반 카메라에서 업체 상세로 이동하고 앱 체크인 스캐너에서는 token 인증에 사용 |
| 25 | Shop 작업 상태 제어 | 체크인된 Shop 예약 상세에서 작업 시작·완료 | Partner만 IN_USE/COMPLETED 전환, 상태 로그와 audit 기록 | 자동 검증 성공·UI 재확인 | User 직접 전환 403 및 Partner audit E2E |
