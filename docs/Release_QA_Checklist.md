...
| 37 | SELF 공통 안전교육 | 미완료 계정으로 SELF 안전 동의 진입 | 최초 1회 공통교육 완료 전 결제 진행 불가, 완료 후 원래 예약으로 복귀 | 자동 검증 성공·배포 재확인 | 콘텐츠 버전별 완료 테이블과 예약 생성 서버 검증 적용 |
...
| 28 | SELF 작업 확인 제공 설정 | 패키지/가격 섹션에서 작업별 제공 상태 변경 | 사용자 작업 확인 옵션에 즉시 반영되고 미제공 작업 포함 시 전체 옵션 비활성 | 구현 완료·배포 재확인 | DB/RLS/API 및 Partner 화면 적용, active membership 정비소만 수정 |
| 29 | SELF 작업 확인 결과 등록 | 작업 확인 예약 상세에서 1차/재확인 저장 | 작업·항목별 결과와 메모 저장, 예약 상태는 변경되지 않음 | 자동 검증 성공·배포 재확인 | 실제 항목별 1차 결과 저장과 User 조회 E2E 통과, 담당 정비사 이름은 사용자에게 미노출 |
