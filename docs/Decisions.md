...
Decision:
Self Service 체크인은 예약 시작 15분 전부터 예약 종료 시각 전까지만 허용하고, 카 마스터 검수는 예약 단계에서만 선택·선결제한다.

Rules:

- 체크인과 이용 시작 API는 서버 시각으로 허용 구간을 검증한다.
- 허용 시각 전 체크인은 `CHECKIN_NOT_OPEN`, 종료 시각 이후는 `CHECKIN_WINDOW_CLOSED`로 거부한다.
- 예약 생성 API는 기존처럼 과거 시작 시각을 거부한다.
- MVP에서는 예약 연장, SOS, 매장 연락 버튼을 제공하지 않는다.
- 예약 종료 후 1시간 bay buffer는 운영 충돌 방지용이며 사용자 연장 시간이 아니다.
- 카 마스터 검수는 예약 단계에서만 선택하며 기본 5,000원과 작업별 가산 금액을 선결제한다.
- 체크아웃 API는 예약에 저장된 검수 여부·금액만 사용하고 새 검수 요청을 받지 않는다.
- 추가 결제비용은 예약 선결제액을 제외한 실제 초과요금으로 표시한다.
- 이용 중 예약은 사용자 전역 상단 배너에서 다시 진입할 수 있다.

Reason:
체크인 시각 제한이 없으면 예약보다 지나치게 일찍 `IN_USE`로 전환할 수 있고, 체크아웃에서 검수를 다시 선택하면 이미 결제한 검수비와 사후정산 검수비가 혼동된다. 체크인 허용 구간과 검수 선택 시점을 서버에서 고정하고, 운영 buffer와 사용자 이용 시간을 분리해야 정산 근거가 명확해진다.
...
Decision:
Self Service 초과요금의 과금 대상 시간은 최대 60분으로 제한하고, 활성 예약의 목록 분류는 날짜보다 상태를 우선한다.

Rules:

- 예약 종료 이후 초과요금은 1시간 단위로 올림하되 최대 1시간분까지만 청구한다.
- 이용 중 타이머의 초과 표시는 음수가 아니라 `초과 이용`으로 표시하고 60분에서 멈춘다.
- `CHECKED_IN`, `IN_USE` 예약은 `blocked_until`이 지났더라도 `/reservation`의 `진행 중·예정` 탭에 표시한다.
- 과금 상한에 도달해도 예약 상태는 자동 완료하지 않으며 사용자가 체크아웃 증적을 제출해 종료해야 한다.

Reason:
장기간 종료되지 않은 `IN_USE` 데이터가 타이머와 예상 초과요금을 무제한 누적해 비현실적인 금액을 표시했고, 날짜 기반 목록 분류 때문에 이용 중 예약이 지난 이용으로 이동했다. 과금 상한과 상태 우선 분류를 명시해 사용자 표시, 서버 정산, 예약 접근 경로를 일치시킨다.
...
Decision:
완료 상세는 예약 목록 진입과 결제 직후 진입을 구분하고, 차량 정비 이력은 완료 예약을 데이터 원천으로 사용한다.

Rules:

- `/reservation`의 지난 이용에서 `/complete`로 이동할 때만 예약 목록 복귀 버튼을 표시한다.
- 체크아웃·사후정산 직후 도착한 `/complete`에는 복귀 버튼을 표시하지 않는다.
- `/my-car` 정비 이력은 현재 사용자의 `COMPLETED` 예약을 `vehicle_id`별로 묶어 표시한다.
- 작업명은 Self 예약 작업과 Shop package snapshot을 사용하고, 금액은 최종 정산액을 우선한다.

Reason:
같은 완료 화면이라도 결제 직후와 과거 이력 열람은 사용자의 다음 행동이 다르다. 또한 차량 화면의 빈 mock history를 실제 완료 예약과 연결해야 차량별 관리 화면으로서 의미가 생긴다.
...
Decision:
차량 등록의 차종 입력은 모바일 드롭다운으로 제공하고 승용차 외 경상용 차량 분류를 포함한다.

Rules:

- 차종 선택지는 경차, 세단, SUV, 해치백, 왜건, 쿠페, 컨버터블, 미니밴/MPV, 승합차, 픽업트럭, 소형 화물차, 기타로 제공한다.
- 포터 같은 1톤급 차량은 `소형 화물차`로 저장한다.
- 차종 값은 차량 분류 정보이며, 실제 예약 가능 여부는 향후 partner/bay 수용 조건으로 별도 검증한다.
- 연식과 차종 입력은 모바일에서 각각 전체 너비 한 줄을 사용한다.

Reason:
세 개 차종을 가로 버튼으로 배치하면 좁은 iPhone 화면에서 해치백 같은 문자열이 개행되어 선택 UI가 깨진다. 차종 범위를 넓힌 드롭다운은 레이아웃을 안정화하고 승합·화물 차량 사용자도 실제 차량에 가까운 분류를 선택할 수 있게 한다.
...
Decision:
파트너 입점 시 베이별 허용 차종과 최대 허용 공차중량을 받고, 예약 준비와 확정 시 선택 차량과의 호환성을 검증한다.

Rules:

- 기존 한국어 차종 12종을 공통 카탈로그로 사용하며 별도 영문 차종 enum을 만들지 않는다.
- `allowed_vehicle_types = []`는 전체 차종 허용, `max_vehicle_weight_kg = null`은 중량 제한 없음이다.
- 기존 베이는 두 기본값으로 이관해 기존 예약 가능 상태를 유지한다.
- 차량 중량은 공차중량 kg로 저장하며, 중량 제한이 있는 베이에서만 필수다.
- 사용자는 `/my-car`에서 기존 차량의 차종과 중량을 수정할 수 있다.
- 호환되지 않는 active bay는 일정 화면에서 숨기지 않고 비활성화하며 사유를 표시한다.
- Partner-admin은 입점 시 입력한 베이 조건을 조회·정정할 수 있고 변경은 `BAY_COMPATIBILITY_UPDATED`로 감사 기록한다.
- 리프트 규격, 차체 치수, 적재 상태, 수동 승인 기능은 MVP 범위에 포함하지 않는다.

Reason:
소형 화물차까지 차량 등록을 허용하면서 모든 베이를 동일하게 예약 가능하게 두면 실제 리프트 수용 범위를 넘길 수 있다. 기존 베이와 예약 흐름을 유지하면서 차종·공차중량 두 조건만 추가하면 운영 안전성을 높이고, 세부 장비 규격 모델링은 실측 데이터가 확보된 뒤 확장할 수 있다.
...
Decision:
Admin과 Partner-admin은 모바일 사용자 셸과 분리된 공통 데스크톱 대시보드 셸을 사용한다.

Rules:

- 운영 콘솔은 최소 1180px 작업 폭과 240px 고정 사이드바를 사용한다.
- Admin과 Partner-admin은 동일한 내비게이션, 상단 바, 카드 곡률과 색상 기준을 공유한다.
- Admin 메뉴는 현재 라우트를 강조하고 Partner-admin 메뉴는 한 화면의 운영 섹션 앵커로 이동한다.
- Partner-admin의 기존 API, 권한 범위와 작업 모달은 UI 재배치 과정에서 변경하지 않는다.
- 사용자 모바일 앱의 max-width shell과 bottom navigation은 운영 콘솔에 공유하지 않는다.
...
Decision:
예약 증적 HEIC는 웹 미리보기가 가능한 JPEG로 변환하고, 베이 잠금은 예약 상태뿐 아니라 예약 버퍼 종료 시각으로 만료한다.

Rules:

- 신규 HEIC/HEIF 사진은 브라우저에서 JPEG로 변환한 뒤 Storage에 업로드한다.
- 기존 HEIC/HEIF 증적은 Partner-admin에서 브라우저 로컬 변환 후 화면 안에서 표시한다.
- 증적 URL은 있으나 실제 객체가 없거나 변환에 실패하면 깨진 이미지 대신 확인 불가 상태를 표시한다.
- Partner-admin 증적은 다운로드 링크 대신 화면 내 확대 보기로 확인한다.
- 체크인 4장과 셀프 체크아웃 2장 필수 정책은 유지한다.
- `CONFIRMED`, `CHECKED_IN`, `IN_USE` 모두 `blocked_until` 전까지만 베이 비활성화를 막는다.
- 과거 예약은 감사·정산 이력으로 보존하며, 시간이 지났다는 이유만으로 삭제하지 않는다.
- 신규 패키지 요청 소요시간은 5분 단위로 입력하고 Partner-admin에 요청 상태를 표시한다.

Reason:
iPhone HEIC 원본은 Chromium에서 직접 표시되지 않아 다운로드되며, 과거 자동 테스트는 Storage 객체 없이 URL만 남겨 깨진 이미지가 발생했다. 또한 종료 처리가 누락된 오래된 체크인 상태가 베이를 영구 잠그면 현재 영업을 방해한다. 증적 필수 규칙과 예약 이력은 유지하면서 파일 호환성과 운영 잠금의 만료 기준을 명확히 해야 한다.
...
Decision:
Admin의 신규 패키지 생성 요청 승인은 전역 패키지 생성과 요청 업장의 판매 가격 연결을 하나의 운영 작업으로 처리한다.

Rules:

- Admin은 승인 시 영문 소문자와 숫자, 하이픈으로 된 전역 패키지 코드를 입력한다.
- 승인 액션은 요청의 이름, 설명, 소요시간으로 `service_packages`를 생성한다.
- 같은 작업에서 요청 업장의 `partner_package_prices`를 희망 공임과 활성 상태로 upsert한다.
- 패키지 또는 가격 연결에 실패하면 요청을 `FULFILLED`로 변경하지 않는다.
- 성공한 패키지 생성과 업장 가격 연결은 각각 `admin_package_audit_logs`에 기록한다.
- 과거 `FULFILLED` 요청에 해당 코드의 전역 패키지가 없으면 같은 액션으로 누락 생성을 복구할 수 있다.
- 소요시간은 5분 단위, 금액은 1원 단위 정수로 입력하고 서버는 양의 소요시간과 0원 이상 금액을 검증한다.

Reason:
기존 화면은 Admin이 카탈로그와 업장 가격을 별도로 만든 뒤 요청 상태만 완료하도록 요구했지만, UI의 “처리 완료”는 실제 생성까지 끝난 것으로 해석되기 쉬웠다. 승인 작업의 결과를 원자적인 운영 의도에 가깝게 묶으면 완료 상태와 실제 판매 데이터가 어긋나는 일을 줄일 수 있다.
...
Decision:
정비소 사진과 리뷰 사진을 공개 서비스 콘텐츠로 분리하고, 리뷰 작성자는 실명 대신 닉네임으로 표시한다.

Rules:

- Partner-admin은 소속 정비소에 최대 8장의 사진을 등록하고 대표 사진 1장을 지정한다.
- 홈 정비소 카드에는 대표 사진만, 정비소 상세에는 등록 사진 전체를 표시한다.
- 완료 예약 리뷰에는 최대 4장의 사진을 연결한다.
- 정비소 사진과 리뷰 사진은 각각 별도 public Storage bucket을 사용하며 예약 증적과 혼용하지 않는다.
- 사용자 프로필의 닉네임만 리뷰에 공개하고 이름, 이메일, 연락처, 차량번호는 공개하지 않는다.
- 기존 사용자와 신규 사용자는 식별 정보가 없는 기본 닉네임을 받으며 마이페이지에서 변경할 수 있다.
- Auth 사용자가 없는 레거시 리뷰도 이력으로 유지하며 프로필 외래키를 강제하지 않고 기본 닉네임으로 표시한다.
- iPhone HEIC/HEIF 사진은 기존 증적 처리와 동일하게 업로드 전 JPEG로 변환한다.

Reason:
정비소 환경과 장비를 예약 전에 확인하고 사진 리뷰로 이용 경험을 공유하려면 공개 이미지 모델이 필요하다. 다만 예약 증적과 공개 콘텐츠는 보안 목적이 다르고, 실명·연락처를 리뷰에 노출하면 개인정보 부담이 커지므로 저장소와 공개 필드를 명시적으로 분리한다.
...
Decision:
현장 체크인은 Partner별 고정 QR 또는 수동 코드로 사용자가 인증하고, Shop
Service의 작업 상태는 Partner가 전환한다.

Rules:

- 베이별 QR 대신 정비소마다 고정 QR token과 수동 코드를 하나씩 발급한다.
- 사용자는 예약 시작 15분 전부터 앱에서 QR을 스캔하거나 수동 코드를 입력한다.
- 서버는 로그인 사용자, 예약 partner, 체크인 시간과 활성 인증정보를 함께 검증한다.
- Self Service는 도착 인증 후 차량 사진 4장을 제출해야 `CHECKED_IN`이 된다.
- Shop Service는 도착 인증 즉시 `CHECKED_IN`이 되며 사용자는 차량을 인계한다.
- Shop 작업은 Partner-admin이 `CHECKED_IN → IN_USE → COMPLETED`로 전환한다.
- 사용자의 Shop 시작/완료 API 호출은 거부한다.
- 인증정보 재발급은 QR과 수동 코드를 함께 교체하고 감사 로그를 남긴다.

Reason:
베이별 QR은 배치와 교체 비용이 크고 사용자가 표시한 QR을 매장이 다시 읽는
기존 화면은 도착 증명 역할이 약했다. 정비소 고정 자격정보를 사용자가 현장에서
입력하면 설치 부담을 줄이면서 장소 일치 여부를 확인할 수 있다. Shop 작업의
실제 진행 여부는 작업자가 가장 정확히 알기 때문에 시작·완료 권한을 Partner로
분리한다.
...
Decision:
Partner 고정 QR의 일반 카메라 진입과 예약 체크인 진입을 분리하고, 현장 QA는
공개 테스트 매장 대신 지정 예약의 시간을 제한적으로 이동해 수행한다.

Rules:

- 새 Partner QR은 `/partner/[partnerId]`를 열며 활성 QR token을 함께 담는다.
- 업체 상세은 일반 브라우저 진입 직후 query token을 제거한 깨끗한 URL로
  리다이렉트한다.
- 앱 체크인 스캐너는 QR URL에서 token을 추출해 현재 예약의 정비소와 검증한다.
- 과거 `/checkin?partnerToken=...` QR을 일반 카메라로 연 경우 활성 token의
  정비소 상세로 이동하고 임의 예약 정보를 표시하지 않는다.
- 예약 ID 없는 체크인 URL은 체크인 폼을 노출하지 않고 예약 목록으로 이동한다.
- 예약 시작 15분 전에는 예약 완료 CTA를 비활성화하고 체크인 직접 URL도 예약
  완료 화면으로 되돌린다.
- Shop 예약 상세는 Partner가 작업 내용을 확인할 수 있도록 패키지명, 설명,
  소요시간을 표시한다.
- 시간 제한 QA는 `PITNOW_QA_RESERVATION_ID`로 지정한 `CONFIRMED` 예약만
  시작 10분 전 상태로 이동하며, 명시적 확인값과 DB 베이 중복 제약을 요구한다.
- 15분 단위 가상 매장이나 체크인 제한 해제 옵션은 Production에 두지 않는다.

Reason:
고정 QR은 특정 예약을 내포하지 않으므로 일반 카메라로 읽었을 때 임의 체크인
화면을 열면 안 된다. 또한 운영 정책을 우회하는 공개 테스트 매장은 실제 고객의
검색과 예약 규칙을 오염시킬 수 있다. 예약 컨텍스트와 시간 게이트를 양쪽 화면에서
강제하고, 운영자가 명시적으로 선택한 테스트 예약만 이동하는 편이 재현성과 안전성을
함께 확보한다.
...
Decision:
예약 종료 시각까지 체크인되지 않은 확정 예약은 취소가 아닌 `NO_SHOW` 상태로
종결한다.

Rules:

- 서버는 `end_time <= now`인 `CONFIRMED` 예약을 `NO_SHOW`로 전환한다.
- migration 적용 시 이미 종료된 기존 확정 예약도 SYSTEM 상태 로그와 함께
  일괄 backfill한다.
- 전환은 User 예약 조회, Partner 예약 polling, Admin 예약 조회 중 어느 경로가
  먼저 실행되더라도 동일한 공통 상태 전환 함수를 사용한다.
- 조건부 갱신으로 한 요청만 전환에 성공하며 `reservation_status_logs`에
  `CONFIRMED → NO_SHOW`와 종료 시각·판정 시각을 기록한다.
- 종료된 예약에 Partner-admin이 `NO_SHOW` 메모를 남기면 Partner actor로 같은
  전환을 실행한다. 그 외 메모 유형은 예약 상태를 바꾸지 않는다.
- 노쇼는 취소가 아니므로 선결제 payment를 자동 환불하거나 `CANCELLED`로
  변경하지 않는다.
- `NO_SHOW`는 베이 점유 상태에서 제외하며 체크인·완료·리뷰 진입을 허용하지
  않는다.

Reason:
운영 메모만으로 노쇼를 표시하면 User, Partner, Admin 화면의 예약 상태와 베이
점유가 서로 어긋난다. 반대로 노쇼를 자동 취소로 처리하면 결제·환불 정책까지
의도치 않게 바뀐다. 별도 종결 상태와 명시적 상태 로그를 두면 방문하지 않은 예약을
정확히 구분하면서 결제 기록과 운영 판단을 유지할 수 있다.
...
Decision:
Self Service의 선택형 유료 부가서비스 명칭을 `정비사 작업 확인`으로 통일하고,
예약 완료 상태와 분리된 작업별 확인 결과를 기록한다. 안전교육은 공통교육과
작업별 콘텐츠로 분리하고 체크박스 동의 증적을 버전과 함께 저장한다.

Rules:

- `SELF_SERVICE`와 `SHOP_SERVICE`의 예약 타입과 상태 전환은 변경하지 않는다.
- Self 작업 확인을 신청해도 예약은 Shop Service로 전환되지 않는다.
- 사용자 화면에서 `카 마스터 검수`, `검수 가산`, `안전 보증`, `출고 승인`
  표현을 사용하지 않고 `정비사 작업 확인`, `작업 확인 비용`을 사용한다.
- 작업 확인은 차량 전체 상태, 향후 고장, 운행 가능 여부를 보증하지 않는다.
- 작업 확인 기본요금은 예약당 5,000원이며 선택 작업별 가산 비용을 더해
  예약 시 선결제한다.
- 정비소는 Self 작업별로 작업 확인 제공 여부를 설정한다. 선택 작업 중 제공하지
  않는 작업이 하나라도 있으면 해당 예약의 작업 확인 옵션을 비활성화한다.
  일부 작업을 조용히 제외하지 않고 신청 가능한 범위를 결제 전에 명확히 표시한다.
- 작업 카드와 결제 전 화면은 작업별 확인 항목을 표시한다.
- 항목별 결과는 `특이사항 없음`, `이상 발견`, `확인 불가`만 사용한다.
- Partner-admin은 단계별 시작/완료 상태를 전환하지 않고 예약 상세에서 결과와
  메모를 저장한다.
- 담당 정비사 프로필은 만들지 않는다. 결과 저장 Partner-admin 계정은 내부
  감사 목적으로만 자동 기록하며 사용자에게 노출하지 않는다.
- Self 체크아웃은 작업 확인 결과를 기다리지 않고 기존처럼 예약을
  `COMPLETED`로 전환한다.
- 유료 작업 확인 결과가 없으면 `결과 등록 대기`, 결과가 있으면
  `결과 등록 완료`로 파생 표시한다.
- 이상 발견 항목은 예약 이용 시간 안에서 1회 재확인을 포함한다. 추가 확인은
  정비소 재량이며 플랫폼 예약 연장을 의미하지 않는다.
- 정비사가 직접 수정하는 작업과 비용은 작업 확인 서비스에서 분리하고
  `정비소와 별도 협의` 기록만 남긴다.
- 공통 안전교육은 첫 Self 이용 전에 별도 과정으로 1회 완료한다. 콘텐츠 버전이
  변경되면 다시 확인할 수 있다.
- 예약 과정에서는 선택 작업의 안전카드를 모두 확인하고, 활성화된 필수 영상이
  있는 작업만 영상을 추가한다.
- 안전 콘텐츠는 작업 방법 전체나 성공을 교육하지 않고 주요 위험과 금지사항을
  안내한다.
- 캔버스 서명은 신규 예약 흐름에서 사용하지 않는다. 로그인 사용자, 동의 문구,
  선택 작업, 콘텐츠 버전, 동의 시각을 체크박스 동의 증적으로 저장한다.
- AI 생성 영상은 대본, 콘티, 위험 상황 보조 장면에 사용할 수 있으나 실제
  정비 절차와 장비 사용 내용은 정비 전문가 검토와 실제 촬영 자료를 우선한다.

Reason:
작업 확인을 Shop 작업 상태에 결합하면 Self 예약의 책임과 흐름이 흐려지고,
결과를 예약 완료 조건으로 만들면 현장 처리 지연이 bay 반환과 정산을 막는다.
예약 상태와 별도 결과 모델을 사용하면 유료 서비스 이행 여부를 추적하면서도
기존 Self 루프를 유지할 수 있다. 또한 명확한 확인 범위, 결과, 동의 버전을
남겨야 사용자 기대를 제한하고 운영 분쟁 시 당시 계약 내용을 재현할 수 있다.
...
Decision:
복수 SELF 작업의 정비사 작업 확인은 선택 작업 전체가 아닌 정비소가 제공하는
작업의 부분 범위로 신청할 수 있다. 작업별 확인 항목은 Partner가 임의 편집하지
않고 Admin 관리 전역 카탈로그를 원천으로 유지한다.

Rules:

- 선택 작업 중 확인 제공 작업이 하나 이상이면 작업 확인 옵션을 활성화한다.
- 기본 5,000원은 예약당 한 번만 더하고 제공 작업의 가산 비용만 합산한다.
- 사용자에게 결제 전 `확인 포함`과 `확인 제외` 작업을 모두 표시한다.
- 미제공 작업도 SELF 예약 작업에는 포함되지만 작업 확인 비용과 결과 범위에서는
  제외한다.
- 예약 시 실제 과금된 작업만 확인 항목과 가산 비용을 스냅샷으로 저장한다.
- Partner-admin은 작업별 전체 확인 항목을 조회하고 작업 단위 제공 여부만
  변경한다.
- 확인 항목 추가·수정·삭제는 Admin이 전역 카탈로그 버전으로 관리한다.
- 재확인은 1차 확인 결과가 저장된 뒤에만 가능하며 UI와 API에서 모두 강제한다.
- `전체 작업 확인 미진행`은 1차·재확인 결과를 모두 삭제하는 예약 단위
  전역 동작이다.

Reason:
복수 작업 중 한 항목의 미제공 때문에 제공 가능한 다른 확인까지 막으면 사용자
선택권과 정비소의 실제 제공 범위가 불필요하게 축소된다. 반대로 Partner가 확인
문구를 자유롭게 바꾸면 정비소별 의미와 가격 근거가 달라지고 예약 당시 계약
범위를 비교하기 어렵다. 전역 카탈로그와 예약 스냅샷을 유지하면서 제공 가능한
부분만 명시적으로 과금하면 유연성과 운영 일관성을 함께 확보할 수 있다.
...
Decision:
사용자는 복수 SELF 작업 중 정비사 작업 확인을 받을 작업을 개별 선택할 수 있다.
공식 사용자 용어는 모든 화면과 문서에서 `정비사 작업 확인`으로 유지한다.

Rules:

- 정비사 작업 확인을 활성화하면 정비소가 제공하는 선택 작업을 모두 기본
  선택한다.
- 사용자는 결제 전에 기본 선택된 작업 중 원하지 않는 작업을 해제할 수 있다.
- 마지막 확인 작업을 해제하면 정비사 작업 확인 옵션도 함께 비활성화한다.
- 기본 5,000원은 예약당 한 번만 부과하고 실제 확인 대상으로 남은 작업의
  가산 비용만 합산한다.
- 작업 확인을 선택하지 않은 SELF 작업도 예약·안전 동의 범위에는 그대로
  포함되지만 작업 확인 결과 범위에는 포함하지 않는다.
- 서버는 확인 대상이 예약 작업과 Partner 제공 범위의 부분집합인지 검증하고
  클라이언트 금액을 신뢰하지 않는다.
- 결제 후 확인 대상은 예약 당시 항목·단가 스냅샷으로 고정하며 사용자 또는
  Partner가 변경하지 않는다.
- Partner-admin 결과 등록과 사용자 결과 조회에는 실제 결제한 작업만 표시한다.
- 결제 준비 전 열린 구형 화면과의 호환을 위해 확인 대상 필드가 누락된 요청만
  제공 가능한 작업 전체 선택으로 처리한다.

Reason:
복수 작업을 예약했다는 이유만으로 모든 작업의 확인 비용을 강제하면 사용자
의도와 과금 범위가 어긋날 수 있다. 전체 기본 선택은 빠른 예약 경험을 유지하고,
개별 해제와 서버 재검증은 선택권·가격 투명성·예약 당시 계약 재현성을 보장한다.
...
Decision:
MVP의 사용자 직접 취소는 체크인 전 `CONFIRMED` 예약에 한해 허용하고 예약
선결제 금액을 전액 환불한다.

Rules:

- 결제 화면은 24시간·2시간 구간 문구를 사용하지 않고 실제 API 정책을 그대로
  안내한다.
- `CHECKED_IN`, `IN_USE`, `COMPLETED` 예약은 사용자가 앱에서 직접 취소하지
  못하며 운영 확인 대상으로 둔다.
- `NO_SHOW`는 취소가 아니며 자동 환불하지 않는다.
- 서버는 환불액을 공통 계산 함수로 정하고 Toss 취소 요청, FAKE 처리,
  payment metadata에 동일한 금액을 사용한다.
- payment metadata에는 `originalAmount`와 `refundAmount`를 기록해 운영자가
  고지 금액과 실제 처리 금액을 대조할 수 있게 한다.

Reason:
기존 결제 화면은 24시간 전 전액, 2시간 전 50% 환불을 안내했지만 취소 API는
모든 `CONFIRMED` 예약에 전액 환불을 실행해 고지와 실제 금액이 달랐다. 부분환불
정책을 도입하려면 결제 상태와 정산 원장까지 함께 확장해야 하므로, MVP에서는
이미 구현된 체크인 전 전액 환불 정책을 공식화해 화면·API·원장을 일치시킨다.
...
Decision:
정비소의 SELF 및 SHOP 제공 여부를 파트너 운영 속성으로 명시하고, 홈 탐색 결과는
사용자가 선택한 정비 방식에 따라 필터링한다.

Rules:

- `partners.supports_self_service`와 `supports_shop_service`를 각각 독립적으로
  관리한다.
- 활성 베이는 두 방식 모두에서 예약 수용 공간으로 사용될 수 있으므로 제공 방식
  자체를 베이 활성 상태로 추론하지 않는다.
- 활성 SHOP 패키지가 없으면 SHOP 가격은 준비 중으로 표시할 수 있지만, 제공 여부는
  별도 운영 속성을 우선한다.
- 홈의 지도 핀, 결과 개수, 카드 목록과 가격 정렬은 선택한 방식의 정비소만 대상으로
  계산한다.
- 샘플 정비소에는 SELF 전용과 SHOP 전용 사례를 각각 유지해 Release QA에서 필터를
  검증한다.

Reason:
정비소마다 제공 가능한 운영 방식이 다를 수 있는데 가격 강조만 바꾸면 사용자가 실제로
예약할 수 없는 정비소까지 탐색 결과에 남는다. 베이·가격 데이터와 서비스 제공 정책을
분리하면 입점 조건을 명확히 표현하고 향후 운영 중단이나 방식 추가에도 대응할 수 있다.
