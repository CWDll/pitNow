...
test.describe("booking flow smoke", () => {
  test("clicks home booking path through work, schedule, safety, and payment", async ({
    ...
    const partnerCard = page
      .locator("article")
      .filter({ has: page.getByRole("heading", { name: seed.partnerName }) });
    await expect(partnerCard).toBeVisible();
    const homeBookingLink = partnerCard.getByRole("link", { name: "예약하기" });
    ...
    await expect(page.getByRole("heading", { name: "예약 방식 선택" })).toBeVisible();
    ...
    await expect(page).toHaveURL(new RegExp(`/partner/${seed.partnerId}/schedule`));
    await expect(page.getByRole("heading", { name: "시간 / 베이 선택" })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: "안전 동의" })).toBeVisible();
