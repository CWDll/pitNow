...
export default function CancelReservationForm({
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-2 text-2xl font-semibold text-slate-950">예약 취소</h3>
      <p className="mt-2 text-sm leading-6 text-slate-700">
        CONFIRMED 예약만 취소할 수 있습니다. 처리 시 상태가 CANCELLED로 바뀌고
        예약 선결제는 자동 환불 또는 수동 환불 확인 대상으로 전환됩니다.
      </p>
      <div className="mt-4 rounded-2xl border border-amber-200 bg-white p-4 text-sm leading-6 text-slate-700">
        <p className="font-semibold text-amber-700">취소 전 확인</p>
        <p className="mt-1">
          이 작업은 예약 시간을 즉시 해제하고, 결제 장부와 상태 전환 로그에 운영
          취소 기록을 남깁니다.
        </p>
      </div>
      <label className="mt-4 block">
        ...
        <textarea
          ...
          maxLength={500}
          className="mt-2 min-h-24 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-950 outline-none ring-amber-300/30 focus:ring-4"
          placeholder="예: 고객 요청으로 예약 취소"
        />
      </label>
      <label className="mt-4 flex items-start gap-3 rounded-2xl border border-amber-200 bg-white p-4 text-sm leading-6 text-slate-700">
        <input
          type="checkbox"
      ...
      <label className="mt-4 block">
        ...
        <input
          ...
          className="mt-2 h-11 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-950 outline-none ring-amber-300/30 focus:ring-4"
          placeholder="예약 취소"
        />
      </label>
      {error ? (
      ...
      <button
        ...
        className="mt-4 h-11 rounded-2xl bg-amber-500 px-5 text-sm font-semibold text-white transition hover:bg-amber-400 disabled:bg-slate-200 disabled:text-slate-400"
      >
